...
// Функция читает одну конкретную запись rec_id из  БД
func ReadOneRec(rec_id int) ([]models.TodoRecord, error) {
	// последний ReadRec--4
	// Функция читает данные из БД
	// id ReadOneRec--1
	if rec_id < 0 {
		return nil, errors.New(
			"id ReadOneRec--1: недопустимый идентификатор записи")
	}
	fmt.Println("Привет! func ReadOneRec()")

	db_connect, err := sql.Open("sqlite", "ep20231204_todo_cli.db")
	// id ReadOneRec--2:
	if err != nil {
		fmt.Printf("ReadOneRec--2: Не могу открыть БД %v", err)
		fmt.Scanln()
		os.Exit(1)
	}
	defer db_connect.Close()
	...
	query := `
		...
		WHERE id = ?;
	`
	rows, err := db_connect.Query(query, rec_id)
	// id ReadOneRec--3
	if err != nil {
		fmt.Printf("ReadOneRec--3: Не могу прочитать из БД %v", err)
		fmt.Scanln()
		os.Exit(1)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			//id ReadOneRec--4
			fmt.Printf("ReadOneRec--4: Не могу прочитать строку из БД %v", err)
			fmt.Scanln()
			continue
		}
		todoRecords = append(todoRecords, todo)
